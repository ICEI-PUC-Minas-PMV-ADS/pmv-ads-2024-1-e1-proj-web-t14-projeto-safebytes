fetch('../Gersons/tutoriais.json')
    ...
    .then(data => {
        ...
        function tutorialConcluido(tutorialIndex) {
            ...
            if (tutorialIndex !== null && !isNaN(tutorialIndex) && tutorialIndex >= 0) {
                let users = JSON.parse(localStorage.getItem('users')) || [];
        
                const tutorialIndexInt = parseInt(tutorialIndex);
        
                users = users.map(user => {
                    if (user.email === email) {
                        user.tutoriaisConcluidos = user.tutoriaisConcluidos || [];
                        if (!user.tutoriaisConcluidos.includes(tutorialIndexInt)) {
                            user.tutoriaisConcluidos.push(tutorialIndexInt);
                        }
                    }
                    return user;
                });
                ...
                console.error('Índice de tutorial inválido:', tutorialIndex);
            }
        }
